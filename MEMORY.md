...
- REST v3 endpoint tests should use `tests.v3.helpers` (`FakeResponse`, `FakeSession`, `public_client`, `authenticated_client`) and inspect requests with helpers like `last_kwargs`, `last_params`, `last_json`, `request_headers`, `auth_payload`, and `last_payload`. Use `FakeSession([FakeResponse(page), ...])` for sequenced responses.
