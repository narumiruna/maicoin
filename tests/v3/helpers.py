...
class FakeSession:
    def __init__(self, response: object) -> None:
        if isinstance(response, (FakeResponse, Exception)):
            self.responses: list[FakeResponse | Exception] = [response]
        elif isinstance(response, list) and all(isinstance(item, (FakeResponse, Exception)) for item in response):
            self.responses = cast("list[FakeResponse | Exception]", response)
        else:
...
def public_client(session: FakeSession) -> Client:
...
def call_kwargs(session: FakeSession, index: int = -1) -> Mapping[str, object]:
...
def last_json(session: FakeSession) -> Mapping[str, object]:
    return cast("Mapping[str, object]", last_kwargs(session)["json"])


def request_headers(session: FakeSession, index: int = -1) -> Mapping[str, str]:
    return cast("Mapping[str, str]", call_kwargs(session, index)["headers"])


def auth_payload(session: FakeSession, index: int = -1) -> Mapping[str, object]:
